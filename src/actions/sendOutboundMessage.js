...
const sendOutboundMessage = async (sendOutboundParams) => {
  ...
  try {
    const resp = await fetch(
      `https://plugins-2080.twil.io/sendOutboundMessage`,
      options
    );
...
Actions.registerAction("SendOutboundMessage", (payload) => {
  if (!payload.callerId)
    ...
    payload.callerId = manager.workerClient.attributes.smsSendingNumber;
  ...
  if (payload.openChat) {
    ...
    const sendOutboundParams = {
      ...
      // WorkflowSid: process.env.CONVERSATION_WORKFLOW,
      // QueueSid: process.env.CONVERSATION_QUEUE,
      // InboundStudioFlow: process.env.CONVERSATION_STUDIO_FLOW,
    };
    ...
    const sendOutboundParams = {
      ...
      WorkerSid: manager.workerClient.sid,
      WorkerFriendlyName: manager.user.identity,
      // WorkspaceSid: process.env.WORKSPACE_SID, // we need this to lookup task if there is a active conversation already
      // WorkflowSid: "",
      // QueueSid: "",
      // InboundStudioFlow: process.env.CONVERSATION_STUDIO_FLOW,
    };
